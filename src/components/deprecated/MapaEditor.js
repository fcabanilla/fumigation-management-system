...
const DrawingControls = ({
  ...
  const map = useMapEvents({
    ...
    dblclick: e => {
      ...
      console.log(
        'Polígono finalizado con doble click. Estadísticas finales:',
...
const MapaEditor = ({ geometry, onChange, readonly = false, height = 400 }) => {
  ...
  const handleStartDrawing = mode => {
    console.log(
      'handleStartDrawing llamado con modo:',
      mode,
      'Estado actual:',
      drawingMode
    );
  ...
  const handleConfirmPolygon = () => {
    console.log(
      '🔵 BOTÓN CONFIRMAR CLICKEADO - Puntos actuales:',
      currentDrawing.length
    );
    ...
    if (onChange) {
      onChange(newGeometry);
      console.log(
        '✅ onChange llamado exitosamente con geometría:',
        newGeometry
      );
    } else {
    ...
    <EditorContainer>
      ...
      {!readonly && (
        <ToolbarContainer>
          ...
          {drawingMode && (
            <ToolButton onClick={handleUndoDrawing}>
              <FaTimes />
              Cancelar Todo
            </ToolButton>
          )}
                ...
                {drawingMode === 'polygon' ? (
                  <>
                    ...
                    {currentStats.status === 'two_points' &&
                      ` - Distancia: ${currentStats.distanciaTotal}km`}
                    {currentStats.status === 'polygon' &&
                      ` - ${currentStats.area}ha ✓`}
                  </>
                ) : (
      ...
      {!readonly && drawingMode === 'polygon' && currentDrawing.length > 0 && (
        <div
          style={{
            background:
              currentStats.status === 'polygon' ? '#f8f9fa' : '#fff3cd',
        ...
        >
          {currentStats.status === 'first_point' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span>
                <strong>Primer punto agregado.</strong> Haz click en otro lugar
                para continuar.
              </span>
            </div>
          )}
